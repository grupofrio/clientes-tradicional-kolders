...
| **PR 5** | `feat/rewards-canje-automatico` | `POST /api/rewards/redeem` + flujo `/rewards/redeem/[id]` (confirmación, folio, instrucciones de entrega, todos los estados de error del doc 03 §8) + webhook n8n a operación + idempotencia. | `api/rewards/redeem` 🆕, `rewards/redeem/[id]/page.tsx` 🆕 | **Alto** (escritura en Odoo) | **Método atómico `gf_loyalty_redeem` en Odoo (Sebas) + decisiones D del cuestionario + S/N Yamil a reglas de canje** |
...
4. Para PR 5: **S/N explícito a las reglas de canje** (bloqueo por vencidos sí/no, límites por periodo, política de reversa, modo de entrega por recompensa).
